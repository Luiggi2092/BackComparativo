...
*/   
    ...
    const [result1,result2,result3,result4,result5,result6,result7] = await Promise.all([
        pool.request().input('ot', sql.Char, req.params.ot).query(`select Orden as "CODMAT",Concepto as "MAT",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='CODMAT' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char, req.params.ot).query(`select Orden as "CODPLAN",Concepto as "PLANCHA",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='PLANCHAS' and Concepto <> '** PLANCHAS **' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char, req.params.ot).query(`select Orden as "CODTIN",Concepto as "TINTA",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='TINTA' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char,req.params.ot).query(`select Orden as "CODBAR",Concepto as "BARNIZ",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='BARNIZ' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char, req.params.ot).query(`select Orden as "CODACA",Concepto as "ACABADOMANUAL",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='Actcod' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char, req.params.ot).query(`select Orden as "CODACA",Concepto as "ACABADOMANUAL",Elemento as "Elemento",Tipo as Tipo,TipoDet as 'TipoDet',FORMAT(CAST(Cantidad AS DECIMAL(18, 2)), '0.00') as "CANTIDAD",FORMAT(CAST(PrecioUni AS DECIMAL(18, 2)), '0.0000') AS "COSTOUND" from produccion.dbo.OrdTCosto where OdtCod=@Ot and Tipo='Amacod' ORDER BY Orden ASC`),
        pool.request().input('ot', sql.Char,req.params.ot).query(`select OdtDescrip as "PRODUCTO",OdtCod as "OT",(Case OdtMon when 'N' THEN 'NUEVOS SOLES' WHEN 'D' THEN 'DOLARES AMERICANOS' else 'NUEVOS SOLES'end) AS "MONEDA" from produccion.dbo.OrdT where OdtCod=@Ot`)
    ]);
...
export const QueryOt = async(req: Request,res:Response) => {
  ...
  try{
      ...
      if(pool){
        ...
        const result = await pool.request().input('ot',sql.Char,ot).query(`select OdtDescrip as 'PRODUCTO' from produccion.dbo.OrdT where OdtCod=@ot`);
